@extends('frontend.layouts.user') @section('title') {{ __('Home') }} @endsection @section('content')
	...
	<div style="background: url(/resources/views/frontend/img2/solarc.png);background-size: 100% 100%;border-radius: 10px; width: 700px; height: 300px;"> <span id="customClosePopup">&times;</span>
		<div style="width: 100%; display: flex; justify-content: center;">
			<div style="margin-top: -10px;">
				...
				<h5 class="orsl">Telegram Chanel: <a href="https://t.me/investmentScriptsx">t.me/investmentScriptsx</a></h5> </div>
		</div>
	</div>
</div>
<script>
	...
	<div class="popup-content">
		...
		<div style="display: flex; justify-content: center; widt: 100%;">
			<div style="margin-top: 111px; width: 95%;">
				<div style="display: flex; justify-content: center; widt: 100%;">
					<div style="margin-top: 50px; padding: 10px;"> @if (isset($successMessage))
						<p id="myParagraph" class="success-message">{{ $successMessage }}</p> @elseif (isset($alreadyUsedMessage))
						<p id="myParagraph" class="error-message">{{ $alreadyUsedMessage }}</p> @elseif (isset($invalidCodeMessage))
						<p id="myParagraph" class="error-message">{{ $invalidCodeMessage }}</p> @endif </div>
				</div>
				<form class="custom-form" method="POST" action="/h5/index.php/home" style="margin-bottom: 30px;"> @csrf
...
<div style="text-align: center; background: url(/resources/views/frontend/img/aertsad.jpg);background-size: 100% 100%; padding: 20px 20px 0 20px; border-radius: 10px; box-shadow: 0 0 30px rgba(44, 46, 48, 0.5);">
	<div style="display: flex; justify-content: center; align-items: center;">
		<div style="text-align: center;">
			...
			<p class="rtt">{{ $currencySymbol }}{{ $dataCount['total_deposit'] }}</p>
		</div>
		<div class="uzun-cizgi"></div>
		<div style="text-align: center;">
			<p class="rtf">Total Investment</p>
			<p class="rtt">{{ $currencySymbol }}{{ $dataCount['total_investment'] }}</p>
		</div>
		<div class="uzun-cizgi"></div>
		<div style="text-align: center;">
			<p class="rtf">total profit</p>
			<p class="rtt">{{ $currencySymbol }}{{ $dataCount['total_profit'] }}</p>
		</div>
		<div class="uzun-cizgi"></div>
		<div style="text-align: center;">
			<p class="rtf">Total Withdrawal</p>
			<p class="rtt">{{ $currencySymbol }}{{ $dataCount['total_withdraw'] }}</p>
		</div>
	</div>
	<div style="display: flex; justify-content: center; align-items: center;">
		<div style="text-align: center;">
			<p class="rtf">Total Reference</p>
			<p class="rtt">{{ $dataCount['total_referral'] }}</p>
		</div>
		<div class="uzun-cizgi"></div>
		<div style="text-align: center;">
			<p class="rtf">Total Team Wins</p>
			<p class="rtt">{{ $currencySymbol }}{{ $dataCount['total_referral_profit'] }}</p>
		</div>
		<div class="uzun-cizgi"></div>
		<div style="text-align: center;">
			<p class="rtf">Deposit Bonus</p>
			<p class="rtt">{{ $currencySymbol }}{{ $dataCount['deposit_bonus'] }}</p>
		</div>
		<div class="uzun-cizgi"></div>
		<div style="text-align: center;">
			<p class="rtf">Investment Bonus</p>
			<p class="rtt">{{ $currencySymbol }}{{ $dataCount['investment_bonus'] }}</p>
		</div>
	</div>
</div>
...
<div style="margin-bottom: 40px;"></div> @section('script')
<script>
function copyRef() {
	/* Get the text field */
...
</script> @endsection @include('frontend.footer.home') @endsection