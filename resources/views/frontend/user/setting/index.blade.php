...
<div style="text-align: center; padding-left: 5px;
    ...
    <div style="flex: 1;">
        <p class="ppr">Balance({{ setting('currency_symbol','global') }})<br><a class="ppa">{{ $user->balance }}</a></p>
    </div>
    <div style="flex: 1;">
    ...
    <div style="flex: 1;">
        <p class="ppr">Profit({{ setting('currency_symbol','global') }})<br><a class="ppa">{{ $user->profit_balance }}</a></p>
    </div>
</div>
</div>
...
<div class="centered-content2">
    <p class="kjk">ID: {{ str_replace('@id.com', '', $user->email) }} </p>
    
    <p class="kjk">Phone: {{ preg_replace('/\+\d+/', '', $user->phone) }} </p>
</div>
