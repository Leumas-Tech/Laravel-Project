...
Route::get('h5/index.php', function () {
    return redirect('/member-login.html');
})->name('h5/index.php');

// Lucky Spin için controller'ı kullanabilmek için bir route ekleyin
Route::post('/lucky-spin', [LuckySpinController::class, 'addToBalance']);

Route::view('/apps', 'frontend.app');
...
|
*/ 
    Route::get('/checkin', [DailyBonusController::class, 'claimBonus'])->middleware('auth');
    
    
    Route::view('/lucky-spin', 'frontend.extra.luckyspin');
    
    
    
    Route::view('/check-in', 'frontend.extra.checkin');
    
    Route::post('/h5/index.php/home', [GiftController::class, 'updateBalance']);
...
// ======================================  User Part =======================================================
Route::group(['middleware' => ['auth', '2fa', 'isActive', 'translate', setting('email_verification', 'permission') ? 'verified' : 'web'], 'prefix' => '/h5/index.php', 'as' => 'user.'], function () {
    //dashboard
    ...
    Route::get('vip', [SchemaController::class, 'index'])->name('schema');
    Route::get('vip-buy/{id}', [SchemaController::class, 'schemaPreview'])->name('schema.preview');


    Route::post('invest-now', [InvestController::class, 'investNow'])->name('invest-now');
    Route::get('my-vip', [InvestController::class, 'investLogs'])->name('invest-logs');
    ...
    Route::get('team', [ReferralController::class, 'referral'])->name('referral');

    Route::get('ranking-badge', [UserController::class, 'rankingBadge'])->name('ranking-badge');


    //settings
    Route::group(['prefix' => 'menu', 'as' => 'setting.', 'controller' => SettingController::class], function () {
...
Route::get('/wallet', function () {
  return view('frontend.user.setting.wallet');
});
